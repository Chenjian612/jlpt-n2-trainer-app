...
### 2026-08-07 `AI-M0 结构化错题讲解闭环`

- 为 300 道文法题和 500 道词汇题建立 `questionId -> testedPointId -> knowledge` 精确映射。
- 新增本地 RAG 讲解服务，输出考点、正误原因、选项证据、复盘建议、可信度和来源；知识未命中时明确拒答。
- 错题回收页升级为 RAG 讲解面板，区分本地知识库、AI 服务和历史缓存。
- AI 服务增加无题库数据的真实模型连通探针 `/health/ai`，并修复本地 Python HTTPS 证书链和代理请求头兼容问题，便于展示前自检。
- 新增 FastAPI 服务，支持健康检查、精确知识检索和 OpenAI-compatible 模型可选润色；答案、考点和来源不可由模型改写。
- 扩展本地讲解缓存并兼容旧缓存结构。
- 新增 3 个前端 RAG 测试和 3 个 Python 知识服务测试，同时修复旧进度测试依赖固定历史日期的问题。

### 2026-08-07 `补充 RAG 联网检索边界`

- 明确 RAG 的检索来源不必是互联网，第一阶段采用经过整理和校对的封闭知识库。
- 补充第一阶段不直接全网搜索的准确性、稳定性、可复现性、安全和版权原因。
- 将“受控 Web RAG”列为后续阶段：仅在本地证据不足时查询可信来源白名单，并保留 URL、访问时间和失败回退信息。

### 2026-08-06 `AI RAG 学习平台方案基线`

本轮不是代码功能开发，而是对后续 AI 方向进行方案收敛和文档同步。

已确认：

- 原有文法、词汇、读解、听力、错题回收和首页推荐继续保留，作为学习业务底座。
- AI 不作为独立聊天机器人，而是作为现有练习模块的增强层。
- 后续方向调整为“小而精的 JLPT N2 RAG 智能错题讲解系统”。
- 第一阶段优先做文法和词汇错题讲解，再扩展到读解原文定位和听力陷阱分析。
- 技术主线为结构化知识库、BM25、Chroma、Rerank、FastAPI AI 服务、来源引用和讲解记录。

新增/更新文档：

- 新增 [AI-RAG-LEARNING-PLAN.md](./AI-RAG-LEARNING-PLAN.md)，作为新的 AI 方案基线。
- 更新 README、ROADMAP、DESIGN-AI、CONTENT-SPEC 和数据规范，避免继续以早期客户端直调 AI 的方案为准。
