...
- README、路线图、状态报告和移动发布说明统一链接该清单，避免后续优先级分散或口径不一致。

### 2026-09-20 `核心模式浏览器回归补齐`

- 新增文法/词汇刷题完整五题流程、文法/词汇学习包完整流程和文法 200 条十题排序闯关回归。
- 新增流程不仅检查结果页，还读取浏览器本地状态，确认训练轮次真正持久化。
- Chromium 回归由 11 条增加到 16 条，核心模式、统一调度、首页联动和听力链路均纳入生产导出验收。

### 2026-09-20 `移动发布配置自动验收`

- 新增 `npm run test:mobile-config`，自动检查两端应用标识、版本号、录音权限禁用、Expo 插件、图标资源和 EAS 构建档案。
- 移动配置验收已接入 GitHub Actions；首次 EAS 项目关联前明确提示待生成 `projectId`，不把尚未登录误报为配置失败。

### 2026-09-20 `AI-M2 质量评估加固`

- 固定评估集现在会逐项校验题号、考点、误选答案、上下文配对和文法/词汇分布，避免题库更新后继续使用陈旧基线。
- 评估报告新增评估集指纹、模型与价格元数据、失败原因分布、成功/回退延迟以及首次/重复错误分组指标。
- 评估命令支持显式质量门槛；回退率、结构失败率、事实锁定、个性化、迁移题质量、P95 延迟或总成本不达标时会写出报告并返回失败状态。
- 对比例阈值、延迟、成本与模型价格增加范围校验，避免无效参数导致质量门槛失真。

### 2026-08-25 `AI-M4 受控 Web RAG`

- 新增审批来源目录，默认仅包含 JLPT 官方练习资料入口、考生样题、教师说明和 FAQ 普通 HTML 页面，不使用开放搜索引擎。
- 新增显式 `ai:web-sync` 同步流程；普通查询不会临时爬网。同步器限制 HTTPS、白名单主机、公网解析地址、重定向目标、HTML 类型和响应大小，并清除脚本、样式与常见提示注入文本。
- Web 缓存保存 URL、UTC 访问时间和内容哈希，并校验审批目录、URL、哈希与过期时间；缓存文件不会进入 Git。
- `/knowledge/search` 仅在本地证据不足、请求显式允许且审批缓存有效时返回独立 `webSources`，网络结果固定为补充用途，不能覆盖本地事实。
- 新增 `/knowledge/research` 受控回答接口；模型引用 ID 必须来自服务端锁定证据，伪造引用或模型失败时回退到摘录回答，无任何证据时返回 404。
- 健康检查新增 Web RAG 开关与有效缓存数量；同步失败、缓存失效和查询无匹配均有明确状态与本地回退。
- 补充 SSRF、私网地址、非白名单、内容清洗、提示注入、哈希篡改、缓存过期、来源撤销、本地优先、Web 补充、锁定引用和拒答测试。

### 2026-08-25 `AI-M3 轻量混合检索基线`

- 为 800 道文法/词汇题建立中日混合 BM25 与本地稀疏 TF-IDF 向量索引，覆盖题干、选项、讲解、复盘提示、标签和来源字段。
- 新增 `POST /knowledge/search`，支持自然语言查询、模式过滤和结果数量限制，返回考点、原题、讲解片段、来源与排序分数。
- 融合 BM25、TF-IDF 余弦相似度与确定性重排信号，返回归一化总分、分项得分，并通过 `matchReasons` 暴露命中依据。
- 将 15 篇读解的 60 道题加入混合索引；读解命中结果返回文章、段落编号、真实段落原文和题库引用，兼容带段号与仅有引用两种证据格式。
- 修正 4 条无法在原文回查的读解证据改写，统一为真实原文引用；新增 60 题全量可追溯性校验。
- 将 23 个听力案例的 26 道题加入混合索引；返回场景、对话、作答依据、关键信号、陷阱和听题清单。
- 听力证据明确区分官方音频转写、文字对话原句、即时应答和教学性摘要，避免把模拟或翻译内容错误标记为逐字转写；26 题均通过结构完整性校验。
- 新增跨模块薄弱点摘要，统一聚合未掌握错题、活跃读解/听力信号和学习包不稳项，输出四项能力的未稳数量、累计暴露次数和优先级。
- 首页薄弱点卡片新增四项能力分布，并展示迁移验证正确率与最近仍需重试的考点数量；已掌握和已解除弱点不会进入统计。
- 新增可选 OpenAI-compatible 语义 Embedding：显式构建 886 条文档向量缓存，按题库指纹和模型校验有效性，并将语义相似度纳入混合排序。
- 普通检索不会自动批量外发题库；Embedding 未配置、缓存失效或查询请求失败时自动保持 BM25 + TF-IDF 本地回退。健康检查会返回配置、索引与实际检索模式。
- 保留证据不足边界：无词项命中时返回空结果，不调用模型猜测或补写知识。
- 新增检索索引、混合分词、精确考点置顶、中文意图检索、稳定排序、模式过滤、空结果和 10 个固定考点评估测试。

### 2026-08-14 `AI-M2 固定质量评估与成本统计`

- 新增 40 案例固定评估集，文法/词汇各半，并覆盖首次错误和重复错误两组学习上下文。
- 新增 AI 辅导生成遥测，区分未配置、请求失败和结构校验失败，记录端到端响应耗时与模型 token 用量。
- 新增可重复运行的质量评估器，统计回退率、结构校验失败率、锁定字段保持率、个性化依据命中率、迁移题质量和按模型单价估算的成本。
- 迁移题质量检查覆盖同考点、新题干、选项唯一、答案索引、解释非空和考点锚点；重复选项现在会直接触发结构回退。
- 新增 3 个 Python 测试，覆盖固定集平衡性、指标聚合、成本计算和模型 usage 采集。

### 2026-08-12 `AI-M1 个性化辅导与迁移验证`

- 新增 FastAPI `POST /tutor/wrong-answer`，基于知识库事实、真实误选项、累计错误和同类错题上下文生成个性化辅导。
- 新增独立 `PersonalizedTutorExplanation` Schema，输出错误诊断、三步判断路径、易混点对比、复习动作、个性化依据和迁移题。
- 服务端与客户端均使用白名单合并：考点、误选项、错误次数、薄弱点统计和迁移题考点由程序锁定，不接受模型伪造。
- 错题页拆分“RAG 事实讲解”和“AI 个性化辅导”，新增迁移题作答、即时反馈和重新生成入口。
- 优化用户可见标签：个性化模块标记为“智能辅导”；模型增强成功时不再显示额外来源徽标，降级时仍显示“本地知识库”或“历史缓存”。
- 新增个性化缓存上下文版本；错误次数、误选项、同类错题数量或迁移结果变化时不复用旧分析。
- 新增迁移结果持久化和旧进度自动迁移，最多保留最近 100 条迁移记录。
- 新增个性化字段锁定测试；当前累计 44 个前端/业务测试和 6 个 Python 测试。

### 2026-08-12 `个性化 AI 辅导设计基线`

- 明确当前 AI 能力主要是基于知识证据的受控润色，知识库讲解与 AI 讲解差异不足，不能把文案变化当作智能辅导效果。
- 将下一阶段定位为“基于可追溯知识库和个人错题历史的自适应 AI 辅导系统”。
- 确认知识库负责答案、考点、选项事实和来源，AI 负责个性化错误诊断、三步判断路径、易混点对比、复习动作和迁移题。
- 新增 [个性化 AI 错题辅导设计](./AI-PERSONALIZED-TUTOR-DESIGN.md)，补齐输入上下文、输出 Schema、接口、缓存、模型边界、迁移题校验、评估指标和实施顺序。
- 调整 AI 路线图：先完成个性化诊断与迁移验证，再推进 BM25、向量检索、Rerank 和跨模块增强。
...
- 根据真实模型生成耗时，将完整错题讲解的后端超时调整为 45 秒、前端等待调整为 55 秒，并扩大结构化 JSON 输出额度，避免长讲解被提前中断。
