...
module.exports = { name: 'reviewSchedule', tests: [
  { name: 'unifies eight review modes and keeps oldest pending work first without mutation', run() {
    const state = { ...createDefaultProgressState(),
      ...
      studyWeaknesses: [
        ...
        study('s200', 'grammar_200', '2026-09-07T00:00:00Z'),
      ],
    ...
    assert.equal(tasks.reduce((sum, task) => sum + task.count, 0), 8);
