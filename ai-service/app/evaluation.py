...
def _validate_evaluation_set(cases: object) -> None:
    ...
    for index, case in enumerate(cases):
        ...
        mode_id = question["modeId"]
        if mode_id not in mode_counts:
            raise ValueError(f"{case_id}: unsupported evaluation mode {mode_id}")
...
def evaluate_quality_gates(summary: dict, gates: dict[str, float]) -> dict:
    ...
    for metric, threshold in gates.items():
        ...
        if not math.isfinite(threshold) or threshold < 0:
            raise ValueError(f"quality gate {metric} must be a finite non-negative number")
        if metric in {
            "fallbackRate",
            "validationFailureRate",
            "lockedFieldsValidRate",
            "personalizationValidRate",
            "averageTransferQualityScore",
        } and threshold > 1:
            raise ValueError(f"quality gate {metric} must be between 0 and 1")
